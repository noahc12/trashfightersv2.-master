...
eloc = []
eloc = [200,20]
enemy_list = Level_01.bad( 1, eloc )
...
enemy   = Enemy(20,200)# spawn enemy
enemy_list = pygame.sprite.Group()   # create enemy group
enemy_list.add(enemy)                # add enemy to group
...
while not done:
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_UP:
                ...
                if event.key == pygame.K_SPACE:
                    ...
                    bull.rect.x = player.rect.x
                    bull.rect.y = player.rect.y
                    active_sprite_list.add(bull)
    ...
    for hit in hits:
        ...
        active_sprite_list.remove(bullets_list)
        score += 1
        print(score)
        if score == 10:
            enemy.kill()
    ...
    for enemyHit in enemyHits:
        for bullets in enemBullets:
            bullets.kill()
        enemBullets.remove(enemBullets)
        active_sprite_list.remove(enemBullets)
        enemScore += 1
        print(enemScore)
        if enemScore == 3:
            player.kill()
